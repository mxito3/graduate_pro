...
from utils.helpers import pretty_bignum, padded_hex, EasyCopy, assure_dir_exists
import logging
import json
import traceback

from utils.signatures import make_abi, get_func_name

from utils.supplement import fetch_sig

from utils.helpers import colorize, COLOR_GRAY, find_f_list, find_f, ENDC
import secret
...
class Loader(EasyCopy):
    ...
    def find_sig(sig, add_color=False):
        ...
        if 'params' in a:
            res = '{}({})'.format(a['name'],', '.join([colorize(x['type'], COLOR_GRAY, add_color)+' '+x['name'][1:] for x in a['params']]))
        else:
            res = a['folded_name']
    ...
    def run(self, VM):
        ...
        try:
            ...
            # and running VM in a special mode that returns 'funccal'
            # in places where it looks like there is a func call

            trace = VM.run(0)

            def func_calls(exp):
                if exp ~ ('funccall', :fx_hash, :target):
                    return [(fx_hash, target)]
                else:
                    return []

            func_list = find_f_list(trace, func_calls)

            for fx_hash, target in func_list:
                self.add_func(target=target, hash=fx_hash)
    ...
    def add_func(self, target, hash=None, name=None):
